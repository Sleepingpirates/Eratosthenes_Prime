...
const int N = 1e8 + 1;
// This creates a std::vector of boolean values representing whether 
...
void sieve_of_eratosthenes(int start, int end) {
    for (int i = 2; i <= sqrt(end); i++) {
        if (is_prime[i]) {
            int start_idx = std::max(i * i, (start + i - 1) / i * i);
...
int main() {
    ...
    std::thread one (sieve_of_eratosthenes, 2, N/8);
    std::thread two (sieve_of_eratosthenes, N/8 + 1, 2*N/8);
    std::thread three (sieve_of_eratosthenes, 2*N/8 + 1, 3*N/8);
    std::thread four (sieve_of_eratosthenes, 3*N/8 + 1, 4*N/8);
    std::thread five (sieve_of_eratosthenes, 4*N/8 + 1, 5*N/8);
    std::thread six (sieve_of_eratosthenes, 5*N/8 + 1, 6*N/8);
    std::thread seven (sieve_of_eratosthenes, 6*N/8 + 1, 7*N/8);
    std::thread eight (sieve_of_eratosthenes, 7*N/8 + 1, 8*N/8);
    ...
    unsigned int sum_primes = std::accumulate(primes.begin(), primes.end(), 0);
